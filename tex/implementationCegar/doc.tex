...
\objectName{org.sosy\_lab.cpachecker.cpa.value.refiner.ValueAnalysisRefiner},
the option \configOption{cegar.refiner = cpa.value.refiner.ValueAnalysisRefiner} would have to be set.

In \cpaChecker, refinement for multiple CPA's precisions is already implemented.
Since our precision refinement for the \symbolicExecutionCPA\ based on abstract variable assignments (Section~\ref{sec:assignmentRefinement}) is very similar to the refinement of the \valueAnalysisCPA, we refactored it to be able to reuse most code.
