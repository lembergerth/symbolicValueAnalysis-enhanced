...
\end{tikzpicture}
\end{subfigure}
\caption{An example program and a CFA representing it}
\label{fig:exCFA}
\end{figure}
...
The implementation of our presented concepts is performed in \cpaChecker, a verification tool for C programs.
...
If a node has no leaving edges, it is a \emph{final node}. Final nodes represent the program exit.
...
A \emph{concrete state} $c$ is a total function $c: X \cup \{pc\} \rightarrow \integerset$ that assigns a specific value of $\integerset$ to every program variable $x \in X$ and to the program counter $pc$. The program counter $pc$ represents the current location in the program.
...
An \emph{abstract domain} \cite{Beyer2007} $D = (C, \semilattice, \concretization)$ consists of a set of possible concrete states $C$, a semi-lattice $\semilattice$ that describes the abstract states and their possible relation to each other and a concretization function $\concretization : \semilattice \rightarrow 2^C$ which maps each element of $\semilattice$ to a subset of $C$.

A \emph{semi-lattice} $\semilattice = (E, \top, \bot, \lesserEqual, \join)$ consists of a set $E$ of elements, a top element $\top \in E$, a partial order $\lesserEqual\ \subseteq (E \times E)$
and the total function $\join : (E \times E) \rightarrow E$ called \emph{join operator}. The elements $e \in E$ of an abstract domain are called \emph{abstract states}.

Two approaches for software verification are \emph{model checking} and \emph{program analysis}, also called data flow analysis.
...
\item one or more abstract domains to work in and the transfer functions that describe the possible transfers between abstract states,
\item a set of precision that describes the degree of abstraction within the abstract domains,
\item a \emph{merge operator} which controls when two abstract states are merged,
\item a \emph{stop operator} that controls when the exploration of a path is stopped, i.e. when a state is already covered by the existing reached states (this is also called termination check), and
\item a \emph{precision adjustment operator} that can weaken or strengthen an abstract state based on a precision.
\end{enumerate}
These elements are encapsulated in a configurable program analysis \cite{Beyer2008}, which is used by the CPA algorithm.

\subsection{Configurable program analysis}
A \emph{CPA with dynamic precision adjustment} $\cpa = (D, \Pi, \transfer, \cpaMerge, \cpaStop, \cpaPrec )$ consists of:
...
\item
the join operator has to be precise or over-approximating. That means the join of two abstract states always has to represent the same or more concrete states than the union of the concrete states both abstract states represent.
This can be formally expressed as $\forall e, e' \in E: \llbracket e \join e' \rrbracket \supseteq \llbracket e \rrbracket \cup \llbracket e' \rrbracket$, and

\item
if one abstract state $e$ is smaller than another abstract state $e'$, the concrete states it represents must be a subset of the concrete states $e'$ represents. Formally, that is $\forall e, e' \in E : e \lesserEqual e' \Rightarrow \llbracket e \rrbracket \subseteq \llbracket e' \rrbracket$
\end{enumerate}
...
\item
The \emph{transfer relation} $\transfer\ \subseteq (E \times G \times E \times \Pi)$. It assigns to each abstract state $e \in E$ possible abstract successors $e' \in E$ with a precision $\pi \in \Pi$.
...
Two common merge operators are \[\cpaMerge^{sep}(e, e', \pi) = e'\] and \[\cpaMerge^{join}(e, e', \pi) = e \join e'.\]
...
The value $\cpaStop(e, R, \pi) = true$ always has to imply $\llbracket e \rrbracket \subseteq \bigcup_{e' \in R} \llbracket e' \rrbracket$ to ensure soundness.
Two common stop operators are \[\cpaStop^{sep}(e, R, \pi) = \exists e' \in R : e \lesserEqual e'\] and \[\cpaStop^{join}(e, R, \pi) = e \lesserEqual \bigjoin\limits_{e' \in R} e'.\]
...
\item
The precision adjustment $\cpaPrec : E \times \Pi \times 2^{E \times \Pi} \rightarrow E \times \Pi$.
It computes a new abstract state and precision for a given abstract state, a given precision and a given set of abstract states with precision.
It can both strengthen and weaken an abstract state.
The following condition has to hold:
\[\forall e, \hat{e} \in E, \pi, \hat{\pi} \in \Pi, R \subseteq (E \times \Pi) : (\hat{e}, \hat{\pi}) = \cpaPrec(e, \pi, R) \Rightarrow \llbracket e \rrbracket \subseteq \llbracket \hat{e} \rrbracket.\]
...
The \emph{CPA algorithm} displayed in Algorithm~\ref{alg:cpa} uses an arbitrary CPA of this form to verify programs.
...
After initializing \reachedSet\ and \waitlistSet\ with the given sets $R_0$ and $W_0$, the following steps are repeated until \waitlistSet\ is empty:
...
Next, it is checked whether the adjusted abstract state $\hat{e}$ is a target state, i.e. represents any concrete state that violates a property.
This is done by \mbox{$\isTargetStateFunc : E \rightarrow \booleanset$}~(Line~\ref{alg:cpa:isTarget}, Alg.~\ref{alg:cpa}), whose behaviour can be defined arbitrarily.
If $\hat{e}$ is a target state, \reachedSet\ and \waitlistSet\ are returned, both containing $\hat{e}$.

Otherwise, each already reached abstract state $e'' \in \reachedSet$ is individually merged with the new state $\hat{e}$ with precision $\hat{\pi}$.
If the merge weakened $e''$, it and its precision are replaced with the weakened state
and the new precision $\hat{\pi}$ in \reachedSet\ and \waitlistSet\ (Lines~\ref{alg:cpa:mergeStart} -- \ref{alg:cpa:mergeStop}, Alg.~\ref{alg:cpa}).
...
After this it the algorithm continues with the next element in \waitlistSet.
If \waitlistSet\ is empty, there are no more reachable states and the reached set is returned, accompanied by the empty \waitlistSet.

\section{Basic Definition of CPAs used}
The following CPAs are used and extended in this work.
...
\end{enumerate}
The three composite operators~\ref{it:compTransfer}~--~\ref{it:compStop} use the corresponding operators of the contained CPAs $\cpa_1$ and $\cpa_2$ as well as \emph{strengthening operators} $\strengthen_j$ and \emph{compare relations} $\compare_j$ with $1 \leq j \leq 2$. They only alter lattice elements through these components.
...
The composition of CPAs can be used to construct a \emph{\compositeCPA}\ \[\compCPA = (D_\times, \Pi_\times, \transfer_\times, \cpaMerge_\times, \cpaStop_\times, \cpaPrec_\times)\]
...
We extend this definition of \compositeCPA\ to allow the composition of an arbitrary number of CPAs.
\begin{align*}
...
The \emph{\locationCPA}\ \cite{Beyer2008} $\locCPA = (D_\locCPA, \singletonPi, \transfer_\locCPA, \cpaMerge^{sep}, \cpaStop^{sep}, \singletonPrec)$ is a CPA that analyzes the syntactical reachability of program locations.
...
\item
The set $\singletonPi = \{ \singletonPrecision \}$ of a single precision that tracks all information.
...
A \emph{predicate} is a boolean formula using linear-arithmetic expressions and equality with uninterpreted functions.
The \emph{\predicateCPA}\ \cite{BeyerBook} \cite{Beyer2008} uses predicate abstraction \cite{Ball2001} to compute abstract states from a formula $\phi$ and a set $\pi$ of predicates (the precision).
Two different kinds of predicate abstraction exist:
The \emph{cartesian predicate abstraction} $(\phi)^{\pi}_C$ is the strongest conjunction of predicates from $\pi$ that is implied by $\phi$.
The \emph{boolean predicate abstraction} $(\phi)^{\pi}_B$ is the strongest boolean combination of predicates from $\pi$ that is implied by $\phi$.
...
The abstract domain
	...
	The bottom element $\bot = \{false\}$ represents no concrete state.
	An abstract state $r$ is less or equal to another abstract state $r'$, if $r$ contains all predicates of $r'$, formally $r \lesserEqual r'$ if $r \supseteq r'$.
	The join of two abstract states $r, r'$ is defined by $r \join r' = r \cap r'$.

	The concretization function $\concretization$ is defined by $\llbracket r \rrbracket = \{ c \in C |\ c \satisfies \varphi_r\}$.

\item
The precisions of set~$\Pi_\predCPA = 2^P$ describe the precision of an abstract state as a set of predicates. If predicate $p \in P$ is in a precision $\pi$, $p$ is tracked by the analysis when $\pi$ is used.
...
The \emph{\valueAnalysisCPA}\ \cite{Beyer2013} tracks integer values for all program variables with a known value explicitly.
...
\[(v \logicAnd v')(x) = \begin{dcases}
	...
	v'(x) & \text{ if } x \in \defRange(v')
\end{dcases}\]
We define the \emph{definition range} of a partial function $f$ as $\defRange(f) = $\mbox{$\{ x |\ \exists y : (x, y) \in f \}$} and
...
The abstract domain $D_\valCPA = (C, \vallattice, \concretization)$ contains the set $C$ of possible concrete states, the semi-lattice $\vallattice$ and the concretization function $\concretization$.
	...
	This means that an abstract state $v$ is less or equal to an abstract state $v'$ if $v$ contains all variable assignments of $v'$ or restricts them even further.
...
The \emph{\symbolicValueAnalysisCPA}\ (introduced in \cite{Lemberger2015} without dynamic precision adjustment and using a different less-or-equal operator) is an extension to the \valueAnalysisCPA. It introduces \emph{symbolic values} to handle non-deterministic values and expressions of unknown value.
...
\item The abstract domain $D_\symValCPA = {(C, \symvallattice, \concretization)}$ with the set $C$ of concrete states,
	...
	The semi-lattice is defined as $\symvallattice = {(\symvalAssignment, \top_\symValCPA, \bot, \lesserEqual, \join)}$.
	Its elements are \emph{abstract symbolic value assignments} $\symvalAssignment = X \partialFunc \symvalset$ mapping program variables in its definition range to values of
  ...
  and 
	...
	the condition
	\[\forall x \in \defRange(v') : v'(x) \in \symValues \Rightarrow v(x)\] is true for the abstract state resulting from $v'(x)$ by replacing all $i \in \symIds$ occurring in $v'(x)$ with $\aliasFunc(i)$.
...
\item The set $\Pi_\symValCPA = L \rightarrow 2^X$ of precisions. Just like $\Pi_\valCPA$, a precision $\pi \in \Pi$ contains for each program location all program variables of $X$ that are tracked at this location.
...
\item The transfer relation $\transfer_\constrCPA$ contains the transfer $a \gtransfer_\constrCPA a'$ if one of the following is true:
	\begin{enumerate}[label=\alph*)]
		\item $g = (\cdot, \assume(p), \cdot)$, $a' = a \cup p$ and
			$a$ does not contain any variable \mbox{$x \in X$}.
		...
		\item $g = (\cdot, w \assign e, \cdot)$ and $a' = a$. The \constraintsCPA\ only cares about assume edges.
	\end{enumerate}
...
The \emph{\symbolicExecutionCPA}\ \cite{Lemberger2015}\ is the composition of \locationCPA, \symbolicValueAnalysisCPA\ and \constraintsCPA.
...
\item The set~$\Pi_\symExCPA = \Pi_\symValCPA \times \Pi_\constrCPA$ of precisions that contains the individual precisions of the \symbolicValueAnalysisCPA\ and \constraintsCPA. We do not need the precision of the \locationCPA\ because it never changes.
\item The transfer relation $\transfer_\symExCPA$.
	It contains the transfer \mbox{$(l, v, a) \gtransfer_\symExCPA (l', v', a', \pi)$}, if:
	\begin{enumerate}[label=\arabic*.]
	\item \mbox{$l \gtransfer_\locCPA (l', \singletonPrecision)$}.
	\item \mbox{$v \gtransfer_\symValCPA (v', \pi_\symValCPA)$}.
	\item One of the following two is true:
	\begin{enumerate}[label=\alph*)]
		\item $g = (\cdot, \assume(p), \cdot)$ and $\strengthenOp{\constrCPA}{\symValCPA}(a'', v') = a'$ is defined,
			with the existing transfer \mbox{$a \gtransfer_\constrCPA (a'', \pi_\constrCPA)$}.

		\item $g = (\cdot, w \assign e, \cdot)$ and \mbox{$a \gtransfer (a, \pi_\constrCPA)$}.
	\end{enumerate}
	\item $\pi = (\singletonPrecision, \pi_\symValCPA, \pi_\constrCPA)$.
	\end{enumerate}
...
Its concrete implementation depends on the used set of precisions of the \constraintsCPA:
